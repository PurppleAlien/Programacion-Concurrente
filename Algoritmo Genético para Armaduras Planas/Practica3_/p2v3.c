...
void imprimir_pesos_y_aptitud(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, DatosAlgoritmo *datos, float probabilidades[MAX_OBJETOS], float probabilidad_acumulada[MAX_OBJETOS]) {
    ...
    for (int i = 0; i < num_cromosomas; i++) {
        ...
        calcular_peso_y_aptitud(poblacion[i], datos, &peso, &aptitud);
                printf("[%d]\t\t%.6f\t%.6f\t\t%.6f\t\t%.6f\n", i, peso, aptitud, probabilidades[i], probabilidad_acumulada[i]);
    }
    printf("-------------------------------------------------------------\n");
}

// Función para evaluar la población
void evaluar_poblacion(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, DatosAlgoritmo *datos, float aptitudes[MAX_OBJETOS], float probabilidades[MAX_OBJETOS], float probabilidad_acumulada[MAX_OBJETOS]) {
    ...
    for (int i = 0; i < num_cromosomas; i++) {
        ...
        probabilidad_acumulada[i] = (i == 0) ? probabilidades[i] : probabilidad_acumulada[i - 1] + probabilidades[i];
    }
}

// Función para imprimir cromosomas
void imprimir_poblacion(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, DatosAlgoritmo *datos)
{
    for (int i = 0; i < num_cromosomas; i++)
    {
...
void seleccion(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, float probabilidad_acumulada[MAX_OBJETOS]) {
        ...
        case 1: {
            ...
            if (leer_archivo(nombre_archivo, &datos)) {
                ...
                for (int i = 0; i < num_iteraciones; i++)
                {
                    ...
                    imprimir_poblacion(poblacion, num_cromosomas, &datos);
                ...
                for (int iteracion = 0; iteracion < num_iteraciones; iteracion++) {
                    printf("Evaluación de la población - Iteración %d\n", iteracion + 1);
                    evaluar_poblacion(poblacion, num_cromosomas, &datos, aptitudes, probabilidades, probabilidad_acumulada);

                    // Imprimir la evaluación de la población con probabilidades
                    imprimir_pesos_y_aptitud(poblacion, num_cromosomas, &datos, probabilidades, probabilidad_acumulada);

                    // Selección
                    seleccion(poblacion, num_cromosomas, probabilidad_acumulada);
                    ...
                    // Imprimir el arreglo del mejor cromosoma
                    printf("Arreglo del Mejor Cromosoma: ");
                    for (int k = 0; k < LONGITUD_CROMOSOMA; k++) {
                        printf("%d", poblacion[mejor_cromosoma][k]);
                    }
                    printf("\n");
                }
            }
