...
void evaluar_poblacion(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, DatosAlgoritmo *datos, float aptitudes[MAX_OBJETOS]) {
    for (int i = 0; i < num_cromosomas; i++) {
        float peso, aptitud;
        calcular_peso_y_aptitud(poblacion[i], datos, &peso, &aptitud);
        aptitudes[i] = aptitud;
...
void seleccion(int poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA], int num_cromosomas, float aptitudes[MAX_OBJETOS]) {
    // Implementación de la selección por ruleta
    int nueva_poblacion[MAX_OBJETOS][LONGITUD_CROMOSOMA];
    float suma_aptitudes = 0;

    for (int i = 0; i < num_cromosomas; i++) {
        suma_aptitudes += aptitudes[i];
    }

    for (int i = 0; i < num_cromosomas; i++) {
        float punto_ruleta = (rand() / (float)RAND_MAX) * suma_aptitudes;
        float suma_parcial = 0;
        for (int j = 0; j < num_cromosomas; j++) {
            suma_parcial += aptitudes[j];
            if (suma_parcial >= punto_ruleta) {
        ...
        case 1: {
            ...
            if (leer_archivo(nombre_archivo, &datos)) {
                ...
                for (int iteracion = 0; iteracion < num_iteraciones; iteracion++) {
                    ...
                    evaluar_poblacion(poblacion, num_cromosomas, &datos, aptitudes);

                    // Selección
                    seleccion(poblacion, num_cromosomas, aptitudes);
